...
def build_fields(schema: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    date.update(**{**{"widget": "datetime",
         ...
         # ⚠️ لا تُضِف input_timezone هنا. أُضيف في 2026-09-12 لتثبيت توقيت
         # الرياض، فأنتج تاريخًا **خاطئًا بأكثر من سنة**: على متصفّح بتوقيت
         # المحيط الهادئ خرج «2025-07-11» بينما الساعة 2026-09-12. عُزل
         # بتجربة رباعية في متصفّح حقيقي: بإزالته وحده يعود التاريخ صحيحًا.
         # الخيار أحدث من Sveltia 0.179.0 المثبّتة — تعرفه بما يكفي
         # لتحذّر من تعارضه مع picker_utc، ولا تعالجه معالجةً صحيحة.
         # وغيابه غير ضارّ: الإزاحة تتبع جهاز الكاتب لكن اللحظة واحدة،
         # والموقع يعرضها بتوقيته هو.
